package com.aakn.test.hibernate;
...
/**
 * This is copied from io.dropwizard.testing.junit.DaoTestRule
 *
 * The only change here is that we can add resources. I've opened an issue in the dropwizard
 * project. https://github.com/dropwizard/dropwizard/issues/2053
 *
 * A JUnit rule for testing DAOs and Hibernate entities. It allows to quickly test the database
 * access code without starting the Dropwizard infrastructure. <p> Example:
 * <pre><code>
 * {@literal @}Rule
 ...
 * Person wizard = daoTestRule.inTransaction(() -> personDAO.create(new Person("Merlin", "The chief
 * wizard")));
...
public class DAOTestRule extends ExternalResource {
  ...
  /**
   * Creates a new builder for {@link DAOTestRule}, which allows to customize a {@link
   * SessionFactory} by different parameters. By default uses the H2 database in the memory mode.
   *
  ...
  public static class Builder {
    ...
    public DAOTestRule build() {
      ...
      resources.forEach(config::addResource);
